...
`apps/worker/wrangler.toml` の `[env.production]` ブロック（`[[env.production.d1_databases]]` の
`database_name` / `database_id`）を、開発用（デフォルトブロック）の D1 と**別の値**にする。
同じブロックの `[env.production.vars]` `D1_DATABASE_ID` も同じ値を持つので併せて変える。

現状は開発側（`database_name = "line-harness"`）と本番側（`database_name = "line-crm"`）が
**同じ検証用 D1 の `database_id` を指している**。`line-crm` という名前の D1 はアカウントに存在しない。
このまま `--env production` でデプロイすると、本番 Worker が検証用 D1 に読み書きする。

> git 上の `wrangler.toml` は `YOUR_DEV_D1_DATABASE_ID` / `YOUR_D1_DATABASE_ID` のプレースホルダ
> （e791f2c で redact 済み）。実値が入っているのはローカル作業コピーのみ（`git update-index --skip-worktree` 指定）。
> 上の「同じ ID を指している」はそのローカルコピーの状態。
...
クライアントごとに変えるのは **① URL ／ ② キー** の 2 点だけ。
③ サーバー名は **`line-harness` 固定（仕様）**。`.claude/settings.json` の permission キーが
この名前を埋め込んでいるため（§6）、変えると deny が外れる。`command` と `args` は共通。
...
§5 ③ に反してサーバー名を `line-harness-miki` のようにクライアント別に変えてしまった場合、
...
**仕様: サーバー名は `line-harness` 固定（§5 ③）。クライアント差分は URL とキーの 2 点に閉じる。**
そうすれば `.claude/settings.json` を全クライアント共通で使い回せる。
`pnpm check:mcp`（`scripts/check-mcp-permissions.ts`）が `.mcp.json.example` と `.mcp.json` の
`mcpServers` に `line-harness` キーがあること、および `packages/mcp-server` を指す別名キーが
無いことを検査する。
...
照合は `pnpm check:mcp` で機械化した（CI は `--ci` で `.mcp.json.example` のみ、ローカルは `.mcp.json` も検査）。
経緯 → [pre-distribution-checklist.md §7](pre-distribution-checklist.md)
